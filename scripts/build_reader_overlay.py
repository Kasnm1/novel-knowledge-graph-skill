...
def build(data:dict[str,Any],title:str)->str:
    ...
    return f'''<!doctype html><html><head><meta charset="utf-8"><title>{html.escape(title)}</title><style>body{{margin:0;font-family:system-ui;background:#0d1117;color:#e6edf3}}header{{position:sticky;top:0;background:#161b22;padding:10px 18px;border-bottom:1px solid #30363d;z-index:2}}main{{display:grid;grid-template-columns:minmax(0,2fr) minmax(320px,1fr);height:calc(100vh - 58px)}}#text{{overflow:auto;padding:24px;white-space:pre-wrap;line-height:1.9;font-family:ui-serif,serif}}aside{{overflow:auto;border-left:1px solid #30363d;padding:16px}}mark{{background:#8a6d1d;color:#fff;padding:1px 0}}mark[data-eids*=" "]{{box-shadow:inset 0 -3px #58a6ff}}mark.focused-evidence{{outline:3px solid #58a6ff;outline-offset:2px;background:#9b781f}}.card{{border:1px solid #30363d;border-radius:10px;padding:10px;margin:8px 0;background:#161b22}}select{{background:#0d1117;color:#e6edf3;border:1px solid #30363d;padding:6px}}.muted{{color:#8b949e;font-size:13px}}a{{color:#79c0ff}}</style></head><body><header><strong>{html.escape(title)}</strong>　<select id="chapter"></select> <span class="muted">重叠证据会共享高亮区；支持 ?chapter=N&evidence=ID 直接定位</span></header><main><div id="text"></div><aside><h3>本章事件</h3><div id="events"></div><h3>证据与实体</h3><div id="evidence"></div></aside></main><script>const D={raw};const sel=document.getElementById('chapter');const esc=s=>String(s??'').replace(/[&<>]/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;'}}[c]));D.chapters.forEach(c=>{{let o=document.createElement('option');o.value=c.chapter;o.textContent=`第${{c.chapter}}章 ${{c.title||''}}`;sel.appendChild(o)}});function focusEvidence(id){{document.querySelectorAll('mark.focused-evidence').forEach(x=>x.classList.remove('focused-evidence'));const q=`mark[data-eids~="${{CSS.escape(id)}}"]`,node=document.querySelector(q);if(node){{node.classList.add('focused-evidence');node.scrollIntoView({{behavior:'smooth',block:'center'}})}}}}function chapterForEvidence(id){{for(const [chapter,rows] of Object.entries(D.evidence||{{}}))if((rows||[]).some(x=>x.id===id))return Number(chapter);return null}}function render(focusId=null){{const ch=Number(sel.value),c=D.chapters.find(x=>Number(x.chapter)===ch)||{{html:''}},evs=D.evidence[String(ch)]||D.evidence[ch]||[];document.getElementById('text').innerHTML=c.html;document.getElementById('events').innerHTML=(D.events[String(ch)]||D.events[ch]||[]).map(e=>`<div class=card><b>${{esc(e.title)}}</b><div class=muted>${{esc(e.type)}} · ${{esc((e.participants||[]).join('、'))}}</div></div>`).join('')||'<div class=muted>无结构化事件</div>';document.getElementById('evidence').innerHTML=evs.map(e=>`<div class=card><a href="javascript:void(0)" data-eid="${{esc(e.id)}}">${{esc(e.quote)}}</a><div class=muted>${{(D.refs[e.id]||[]).map(r=>esc(r.label)).join(' · ')}}</div></div>`).join('')||'<div class=muted>无证据</div>';document.querySelectorAll('[data-eid]').forEach(a=>a.onclick=()=>focusEvidence(a.dataset.eid));if(focusId)setTimeout(()=>focusEvidence(focusId),0)}}sel.addEventListener('change',()=>render());if(sel.options.length){{const params=new URLSearchParams(location.search),eid=params.get('evidence');let requested=Number(params.get('chapter'));if(!Number.isInteger(requested)&&eid)requested=chapterForEvidence(eid);if(Number.isInteger(requested)){{const option=[...sel.options].find(o=>Number(o.value)===requested);if(option)sel.value=String(requested)}}render(eid)}}</script></body></html>'''


def main()->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--graph',required=True,type=Path);p.add_argument('--chapters-jsonl',required=True,type=Path);p.add_argument('--output',required=True,type=Path);p.add_argument('--cutoff',type=int);a=p.parse_args();g=json.loads(a.graph.read_text(encoding='utf-8'));g=filter_graph(g,a.cutoff,strict=True) if a.cutoff is not None else g;chs=load_chapters(a.chapters_jsonl,a.cutoff);title=str(g.get('metadata',{}).get('title') or 'Novel reader');a.output.parent.mkdir(parents=True,exist_ok=True);atomic_write_text(a.output,build(payload(g,chs),title));print(json.dumps({'output':str(a.output),'chapters':len(chs),'cutoff':a.cutoff,'deep_links':True},ensure_ascii=False));return 0
