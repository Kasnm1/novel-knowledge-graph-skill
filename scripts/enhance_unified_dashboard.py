...
function nkgImportant(e,p){let rows=[];for(const x of p.important_attributes){if(e.attributes&&Object.hasOwn(e.attributes,x.source_key))rows.push({label:x.label||x.source_key,value:e.attributes[x.source_key],reason:x.reason||''})}if(!rows.length)rows=Object.entries(e.attributes||{}).slice(0,6).map(([k,v])=>({label:k,value:v,reason:''}));return rows}
